defmodule NewWebApp.ViteWatcher do
  @moduledoc """
  ...
  def run do
    case System.cmd("pnpm", ["exec", "vite", "dev"],
      ...
      {_output, status} ->
        Logger.error(
          "NewWebApp.ViteWatcher: `pnpm exec vite dev` exited with status #{status} — Vite failed to " <>
    ...
    e in ErlangError ->
      case e do
        %ErlangError{original: :enoent} ->
          Logger.error(
            "NewWebApp.ViteWatcher: pnpm not found on PATH — install pnpm to run the Vite dev watcher."
          )
