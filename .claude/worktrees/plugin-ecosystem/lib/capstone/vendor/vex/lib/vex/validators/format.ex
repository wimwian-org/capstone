defmodule Capstone.Vendor.Vex.Validators.Format do
  @moduledoc """
  ...
  ## Examples
      ...
      iex> Capstone.Vendor.Vex.Validators.Format.validate("foo", ~r/^f/)
      :ok
      iex> Capstone.Vendor.Vex.Validators.Format.validate("foo", ~r/o{3,}/)
      {:error, "must have the correct format"}
      iex> Capstone.Vendor.Vex.Validators.Format.validate("foo", [with: ~r/^f/])
      :ok
      iex> Capstone.Vendor.Vex.Validators.Format.validate("bar", [with: ~r/^f/, message: "must start with an f"])
      {:error, "must start with an f"}
      iex> Capstone.Vendor.Vex.Validators.Format.validate("", [with: ~r/^f/, allow_blank: true])
      :ok
      iex> Capstone.Vendor.Vex.Validators.Format.validate(nil, [with: ~r/^f/, allow_nil: true])
      :ok
  ...
  Custom error messages (in EEx format), provided as :message, can use the following values:
      ...
      iex> Capstone.Vendor.Vex.Validators.Format.__validator__(:message_fields)
      [value: "The bad value", pattern: "The regex that didn't match"]

  An example:

      iex> Capstone.Vendor.Vex.Validators.Format.validate("bar", [with: ~r/"^f"/, message: "<%= value %> doesn't start with an f"])
