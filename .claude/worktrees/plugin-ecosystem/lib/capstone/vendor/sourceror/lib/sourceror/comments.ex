defmodule Capstone.Vendor.Sourceror.Comments do
  @moduledoc """
  Utilities to merge an un-merge comments and quoted expressions.
  """

  import Capstone.Vendor.Sourceror.Identifier, only: [is_pipeline_op: 1, is_binary_op: 1]
  ...
  def merge_comments(quoted, comments) do
    ...
    case leftovers do
      ...
      _ ->
        if match?({:__block__, _, [_ | _]}, quoted) and not Capstone.Vendor.Sourceror.Identifier.do_block?(quoted) do
          {last, args} = Capstone.Vendor.Sourceror.get_args(quoted) |> List.pop_at(-1)
          line = Capstone.Vendor.Sourceror.get_line(last)
          ...
          {:__block__, Capstone.Vendor.Sourceror.get_meta(quoted), args ++ [last]}
        else
          line = Capstone.Vendor.Sourceror.get_line(quoted)
  ...
  defp gather_leading_comments_for_node(quoted, comments) do
    line = Capstone.Vendor.Sourceror.get_line(quoted, 0)
  ...
  defp gather_trailing_comments_for_node(quoted, comments) do
    line = Capstone.Vendor.Sourceror.get_end_line(quoted, 0)
    has_closing_line? = Capstone.Vendor.Sourceror.has_closing_line?(quoted)
  ...
  def extract_comments(quoted, opts \\ []) do
    ...
    quoted =
      if correct_lines do
        Capstone.Vendor.Sourceror.LinesCorrector.correct(quoted)
      else
        quoted
      end
  ...
  defp do_extract_comments({_, meta, _} = quoted, acc, collapse_comments, false) do
    ...
    trailing_comments =
      if collapse_comments do
        quoted |> Capstone.Vendor.Sourceror.get_end_line() |> collapse_comments(trailing_comments)
      else
        trailing_comments
      end
  ...
  defp span(quoted) do
    %{start: range_start, end: range_end} = Capstone.Vendor.Sourceror.get_range(quoted, include_comments: true)
