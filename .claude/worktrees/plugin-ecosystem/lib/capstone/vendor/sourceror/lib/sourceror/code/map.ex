...
defmodule Capstone.Vendor.Sourceror.Code.Map do
  @moduledoc """
  ...
  defp do_put_in_map(zipper, [key | rest], value, updater) do
    cond do
      ...
      Common.node_matches_pattern?(zipper, {:%{}, _, _}) ->
        zipper
        |> Zipper.down()
        |> Capstone.Vendor.Sourceror.Code.List.move_to_list_item(fn item ->
        ...
        |> case do
          ...
          {:ok, zipper} ->
            zipper
            |> Capstone.Vendor.Sourceror.Code.Tuple.tuple_elem(1)
            |> case do
              {:ok, zipper} ->
