...
defmodule Capstone.Vendor.Sourceror.Code.List do
  @moduledoc """
  ...
  def remove_from_list(zipper, predicate) do
    zipper = Capstone.Vendor.Sourceror.Code.Common.maybe_move_to_single_child_block(zipper)
    ...
    if list?(zipper) do
      Common.within(zipper, fn zipper ->
        with zipper when not is_nil(zipper) <- Zipper.down(zipper),
          ...
          {:ok, Capstone.Vendor.Sourceror.Code.Common.replace_code(zipper, value)}
        else
          nil -> :error
          :error -> :error
        end
    ...
    if list?(zipper) do
      Common.within(zipper, fn zipper ->
        with zipper when not is_nil(zipper) <-
               zipper |> Capstone.Vendor.Sourceror.Code.Common.maybe_move_to_single_child_block() |> Zipper.down(),
  ...
  defp do_map(zipper, fun) do
    case Capstone.Vendor.Sourceror.Code.Common.within(zipper, fun) do
      {:ok, zipper} ->
