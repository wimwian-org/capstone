defmodule Capstone.Vendor.Vex.Validators.Length do
  @moduledoc """
   ...
   * `:tokenizer`: A function with arity 1 used to split up a
     ...
     `String.graphemes` and all other values (eg, lists) are
      passed through intact. See `Capstone.Vendor.Vex.Validators.tokens/1`.
   * `:message`: Optional. A custom error message. May be in EEx format
      and use the fields described in "Custom Error Messages," below.

  ## Examples

      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", 3)
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", 2)
      {:error, "must have a length of 2"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate(nil, [is: 2, allow_nil: true])
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("", [is: 2, allow_blank: true])
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", min: 2, max: 8)
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", min: 4)
      {:error, "must have a length of at least 4"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", max: 2)
      {:error, "must have a length of no more than 2"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", max: 2, message: "must be the right length")
      {:error, "must be the right length"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", is: 3)
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", is: 2)
      {:error, "must have a length of 2"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", in: 1..6)
      :ok
      iex> Capstone.Vendor.Vex.Validators.Length.validate("foo", in: 8..10)
      {:error, "must have a length between 8 and 10"}
      iex> Capstone.Vendor.Vex.Validators.Length.validate("four words are here", max: 4, tokenizer: &String.split/1)
      :ok
  ...
  Custom error messages (in EEx format), provided as :message, can use the following values:
      ...
      iex> Capstone.Vendor.Vex.Validators.Length.__validator__(:message_fields)
      [value: "Bad value", tokens: "Tokens from value", size: "Number of tokens", min: "Minimum acceptable value", max: "Maximum acceptable value"]

  An example:

      iex> Capstone.Vendor.Vex.Validators.Length.validate("hello my darling", min: 4, tokenizer: &String.split/1,
  ...
  alias Capstone.Vendor.Vex.Blank
