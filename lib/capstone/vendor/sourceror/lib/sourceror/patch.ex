defmodule Capstone.Vendor.Sourceror.Patch do
  @moduledoc """
  Functions that generate patches for common operations.

  Functions in this module assume that the AST was parsed using Capstone.Vendor.Sourceror
  functions and that it wasn't modified. If you changed the tree before calling
  `Capstone.Vendor.Sourceror.Patch` functions, then the patch ranges are not guaranteed to match
  ...
  If the call is a sigil, you only need to provide the replacement letter:
      ...
      "~F(foo)"
  """
  @spec rename_call(call :: Macro.t(), new_name :: atom | String.t()) :: Capstone.Vendor.Sourceror.Patch.t()
  ...
  @spec rename_identifier(identifier :: Macro.t(), new_name :: atom | String.t()) ::
          Capstone.Vendor.Sourceror.Patch.t()
  ...
  ## Examples
      ...
      "[foo: b, c: d, bar: f]"
  """
  @spec rename_kw_keys(keyword :: Macro.t(), replacements :: keyword) :: [Capstone.Vendor.Sourceror.Patch.t()]
  ...
  defp patch_for_kw_key(quoted, new_key) do
    range =
      quoted
      |> Capstone.Vendor.Sourceror.get_range()
  ...
  ## Examples
      ...
      "bar"
  """
  @spec replace(zipper :: Capstone.Vendor.Sourceror.Zipper.t(), replacement :: String.t()) :: Capstone.Vendor.Sourceror.Patch.t()
  def replace(%Zipper{} = zipper, replacement), do: replace(zipper.node, replacement)

  def replace(ast_node, replacement) do
    range = Capstone.Vendor.Sourceror.get_range(ast_node)
