defmodule Capstone.Vendor.Sourceror.Zipper.Inspect do
  @moduledoc """
  Provides `Capstone.Vendor.Sourceror.Zipper`'s implementation for the `Inspect` protocol.
  ...
  the full path, which can be very verbose for large ASTs.
      ...
      iex> alias Capstone.Vendor.Sourceror.Zipper, as: Z
      Capstone.Vendor.Sourceror.Zipper
  ...
  Zippers can be inspected in these formats:
    ...
    * `:raw` - display the raw `%Capstone.Vendor.Sourceror.Zipper{}` struct including the
      `:path`.
  ...
  Using the zipper defined above as an example:
      ...
      %Capstone.Vendor.Sourceror.Zipper{
        ...
        path: %{
          parent: %Capstone.Vendor.Sourceror.Zipper{
  ...
  def inspect(zipper, :as_code, opts) do
    zipper.node
    |> Capstone.Vendor.Sourceror.to_algebra(Map.to_list(opts))
    |> inspect_opaque_zipper(zipper, opts)
  end

  def inspect(zipper, :raw, opts) do
    open = color("%Capstone.Vendor.Sourceror.Zipper{", :map, opts)
    sep = color(",", :map, opts)
  ...
  defp inspect_opaque_zipper(inner, zipper, opts) do
    ...
    force_unfit(
      concat([
        color("#Capstone.Vendor.Sourceror.Zipper<", :map, opts),
        inner_content,
        line(),
        color(">", :map, opts)
      ])
  ...
  Consider the following zipper:
      ...
      iex> Capstone.Vendor.Sourceror.Zipper.Inspect.default_inspect_as(:as_code)
      :ok
      iex> zipper
      #Capstone.Vendor.Sourceror.Zipper<
        #root
      ...
      iex> Capstone.Vendor.Sourceror.Zipper.Inspect.default_inspect_as(:raw)
      :ok
      iex> zipper
      %Capstone.Vendor.Sourceror.Zipper{
        node: {:def, [line: 1], [{:my_function, [line: 1], nil}, [do: :ok]]},
...
defimpl Inspect, for: Capstone.Vendor.Sourceror.Zipper do
  alias Capstone.Vendor.Sourceror.Zipper, as: Z
