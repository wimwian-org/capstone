defmodule Capstone.Vendor.Sourceror.Zipper do
  @moduledoc """
  ...
  alias Capstone.Vendor.Sourceror.Zipper, as: Z
      ...
      %{node: node} = zipper, acc ->
        ...
        cond do
          ...
          not is_nil(acc) and is_tuple(acc.node) and
              Capstone.Vendor.Sourceror.get_range(acc.node) == node_range ->
            {:cont, zipper, acc}
  ...
  ## Examples
      ...
      false
      iex> follow_path(if_zipper, first_path) |> Capstone.Vendor.Sourceror.Zipper.node() |> Capstone.Vendor.Sourceror.to_string()
      "target()"
      iex> follow_path(if_zipper, second_path) |> Capstone.Vendor.Sourceror.Zipper.node() |> Capstone.Vendor.Sourceror.to_string()
      "target()"
  ...
  ## Examples
      ...
      iex> rewritten
      ...> |> follow_path(path)
      ...> |> then(fn zipper -> replace(zipper, {:wrapped, [], [Capstone.Vendor.Sourceror.Zipper.node(zipper)]}) end)
      ...> |> topmost_root()
  ...
  defp do_traverse_next(%Z{} = next, _zipper, fun) do
  ...
  defp do_traverse_remove(%Z{path: nil} = zipper, _fun) do
    {:removed_root, nearest_supertree_with_path(zipper.supertree)}
  end

  defp do_traverse_remove(%Z{} = zipper, fun) do
  ...
  defp do_traverse_next(%Z{} = next, _zipper, acc, fun) do
  ...
  defp do_traverse_remove(%Z{path: nil} = zipper, acc, _fun) do
    {:removed_root, nearest_supertree_with_path(zipper.supertree), acc}
  end

  defp do_traverse_remove(%Z{} = zipper, acc, fun) do
    zipper = remove(zipper)
    do_traverse_next(next(zipper), zipper, acc, fun)
  end
  ...
  defp nearest_supertree_with_path(%Z{path: nil, supertree: supertree}),
    do: nearest_supertree_with_path(supertree)

  defp nearest_supertree_with_path(%Z{} = supertree), do: supertree

  defp remove_subtree_root_from_supertree(%Z{path: path} = zipper)
