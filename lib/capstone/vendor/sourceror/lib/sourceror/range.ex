defmodule Capstone.Vendor.Sourceror.Range do
  @moduledoc """
  Represents a start/end position in a source file as `:line` and `:column`
  keyword lists.
  """
  import Capstone.Vendor.Sourceror.Identifier, only: [is_unary_op: 1, is_binary_op: 1]
  import Capstone.Vendor.Sourceror.Utils.TypedStruct

  typedstruct do
    field :start, Capstone.Vendor.Sourceror.position(), enforced?: true
    field :end, Capstone.Vendor.Sourceror.position(), enforced?: true
  end
  ...
  """
  @spec new(start_pos :: Capstone.Vendor.Sourceror.position(), end_pos :: Capstone.Vendor.Sourceror.position()) :: t()
  ...
  defp do_get_range({:__block__, meta, args} = quoted) do
    if Capstone.Vendor.Sourceror.has_closing_line?(quoted) do
  ...
  defp get_range_for_node_with_closing_line({:fn, meta, _} = quoted) do
    start_position = Capstone.Vendor.Sourceror.get_start_position(quoted)

    end_position =
      if meta[:end_of_expression] do
        Keyword.take(meta[:end_of_expression], [:line, :column])
      else
        quoted
        |> Capstone.Vendor.Sourceror.get_end_position()
        |> Keyword.update!(:column, &(&1 + 3))
      end
  ...
  defp get_range_for_node_with_closing_line({_, meta, _} = quoted) do
    start_position = Capstone.Vendor.Sourceror.get_start_position(quoted)
    end_position = Capstone.Vendor.Sourceror.get_end_position(quoted)
