...
defmodule Capstone.Vendor.Sourceror.Code.Common do
  @moduledoc """
  ...
  defmacro move_to_pattern(zipper, pattern) do
    quote do
      case Capstone.Vendor.Sourceror.Zipper.find(unquote(zipper), fn
             unquote(pattern) ->
               true
  ...
  def add_comment(zipper, comment, opts \\ []) do
    ...
    node =
      case opts[:placement] || :before do
        :before ->
          Capstone.Vendor.Sourceror.prepend_comments(zipper.node, comments)

        :after ->
          Capstone.Vendor.Sourceror.append_comments(zipper.node, comments)
      end
  ...
  def add_code(zipper, new_code, placement) when is_atom(placement) do
    Logger.warning("""
    Passing an atom as the third argument to `Capstone.Vendor.Sourceror.Code.Common.add_code/3` is deprecated in favor of an options list.

    Instead of:

        Capstone.Vendor.Sourceror.Code.Common.add_code(zipper, new_code, #{inspect(placement)})

    You should now write:

        Capstone.Vendor.Sourceror.Code.Common.add_code(zipper, new_code, placement: #{inspect(placement)})
    """)
  ...
  def add_code(zipper, new_code, opts) when is_binary(new_code) do
    code = Capstone.Vendor.Sourceror.parse_string!(new_code)
  ...
  def replace_code(%Zipper{} = zipper, code) when is_binary(code) do
    replace_code(zipper, Capstone.Vendor.Sourceror.parse_string!(code))
  end
