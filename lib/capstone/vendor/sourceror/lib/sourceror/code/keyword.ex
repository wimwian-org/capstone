...
defmodule Capstone.Vendor.Sourceror.Code.Keyword do
  @moduledoc """
  ...
  def get_key(zipper, key) do
    ...
    if Capstone.Vendor.Sourceror.Code.List.list?(zipper) do
      item =
        Capstone.Vendor.Sourceror.Code.List.move_to_list_item(zipper, fn item ->
          if Capstone.Vendor.Sourceror.Code.Tuple.tuple?(item) do
            case Capstone.Vendor.Sourceror.Code.Tuple.tuple_elem(item, 0) do
              {:ok, first_elem} ->
      ...
      case item do
        {:ok, zipper} -> Capstone.Vendor.Sourceror.Code.Tuple.tuple_elem(zipper, 1)
        :error -> :error
      end
    else
      :error
  ...
  def remove_keyword_key(zipper, key) do
    Capstone.Vendor.Sourceror.Code.List.remove_from_list(zipper, fn zipper ->
      Capstone.Vendor.Sourceror.Code.Tuple.elem_equals?(zipper, 0, key)
    end)
  end
